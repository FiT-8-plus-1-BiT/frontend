      ...
      <div className="pt-[12px] flex flex-col gap-2">
        ...
        <div className="flex flex-wrap gap-2">
          {tags.map((tag, index) => (
            <span
              key={index}
              className="bg-gray-200 text-gray-700 text-m font-semibold px-2 py-1 rounded-md"
            >
              {tag}
            </span>
          ))}
        ...
        {/* 제목 */}
        <h3 className="text-3xl font-semibold line-clamp-2">{title}</h3>

        {/* 강연자 이름 */}
        <p className="text-xl font-medium text-gray-700">{speaker}</p>

        {/* 설명(최대 3줄까지 표시, 초과 시 말줄임) */}
        <p className="text-m text-gray-600 line-clamp-3">{description}</p>
      </div>
    </div>
  );
}
