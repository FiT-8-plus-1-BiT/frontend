...
const QuestionListComponent = ({ sessionId }) => {
  ...
  useEffect(() => {
    const fetchLikes = async () => {
      ...
      setLikeStatusMap(newStatus);
    };

    if (topQuestions.length > 0 && token) {
      fetchLikes();
    }
  }, [topQuestions, token, sessionId]);

  const handleLikeToggle = async (messageId) => {
    const likedByUser = likeStatusMap[messageId];
    try {
      if (likedByUser) {
    ...
    <div className="bg-gray-100 m-2 p-2">
      ...
      <ul className="space-y-3 overflow-y-auto h-[14vw]">
        {topQuestions.map((q) => {
          ...
          const likedByUser = likeStatusMap[q.messageId];
          ...
          return (
            ...
            >
              ...
              <button
                ...
                onClick={() => handleLikeToggle(q.messageId)}
              >
                <Heart
                  className={`w-4 h-4 ${likedByUser ? 'fill-[#262F70]' : 'fill-none'} text-[#262F70]`}
                />
                {q.likes + (likedByUser ? 1 : 0)}
              </button>
            </li>
          );
        })}
